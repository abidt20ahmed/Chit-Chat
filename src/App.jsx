  ...
  return (
    ...
    {loading && messages &&
      messages.map((message, index) => (
        ...
        alt='user'
        />
          <a className={`${message?.link && 'underline text-blue-600'}`} href={message?.link ? message.link : userInput}><span className={`bg-slate-600 py-1 px-3 rounded-xl rounded-bl-none ${!!bold && 'font-bold'} ${!!italic && 'italic'} ${!!strike && 'line-through'} ${message.style && message.style.join(' ')} break-all`}>
        {message.text}
        </span></a>  
        </div>
        ))}
